...
from tensorflow.python.keras.applications.resnet_v2 import preprocess_input, decode_predictions
import numpy as np
import io  # File 객체를 메모리상에서만 이용하도록
import os  # os.path / os.environ
from PIL import Image  # Image 객체
import urllib.request  # 파일 받기
import h5py

# (.h5 경로변경이 추가된 squeezenet.py)
...
def handler(event, context):
    ...
    dynamo_client.put_item(
        TableName='keras-blog-result',  # DynamoDB의 Table 이름
        Item={
            'filename': {
                'S': file_name,
            },
